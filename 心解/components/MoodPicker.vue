...
function selectMood(value) {
  const newVal = props.modelValue === value ? '' : value
  if (newVal) {
    bounceMood.value = value
    setTimeout(() => {
      bounceMood.value = ''
    }, 400)
  }
...
.mood-glow {
  ...
  height: 100%;
  border-radius: $radius-full;
  background: radial-gradient(circle, rgba($color-primary-container, 0.4) 0%, transparent 70%);
...
.mood-selected {
  .mood-circle {
    opacity: 1;
    border: 4rpx solid rgba($color-primary-container, 0.6);
    transform: scale(1.1);
    box-shadow: 0 8rpx 24rpx rgba($color-primary-container, 0.2);
  }
...
.mood-bounce {
  .mood-circle {
    animation: mood-bounce-animation 0.4s cubic-bezier(0.34, 1.56, 0.64, 1);
  }

  .mood-emoji {
    animation: emoji-bounce-animation 0.4s cubic-bezier(0.34, 1.56, 0.64, 1);
  }
}

@keyframes mood-bounce-animation {
  0% { transform: scale(0.7); }
  50% { transform: scale(1.25); }
  100% { transform: scale(1.1); }
}

@keyframes emoji-bounce-animation {
