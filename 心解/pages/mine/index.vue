...
function showMenu() {
  uni.showActionSheet({
    ...
    success: (res) => {
      if (res.tapIndex === 0) uni.navigateTo({ url: '/pages/mine/settings' })
      else if (res.tapIndex === 1) uni.navigateTo({ url: '/pages/mine/feedback' })
    }
  })
}
...
function goPage(page) {
  ...
  if (routes[page]) uni.navigateTo({ url: routes[page] })
}
